...
class CleanSessions(webapp.RequestHandler):
    def get(self):       
        q = model.Session.all()
        q.filter("last_activity < ", datetime.utcnow() - timedelta(days = 1))
        unused_sessions = q.fetch(100)
        for s in unused_sessions:
            s.delete()

application = webapp.WSGIApplication(
                                     [
                                      ('/task/cleanSessions', CleanSessions)
                                     ],
                                     debug=False)
